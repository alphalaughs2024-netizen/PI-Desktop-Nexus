  ...
  return <section className="context-vault-tab">
    <div className="context-vault-workspace">
      <aside className="context-vault-rail" aria-label={t("panel.tabs.contextVault")}>
        ...
        <button className={`context-vault-category${category === "all" && !tag ? " active" : ""}`} onClick={() => { setCategory("all"); setTag(null); }}><span>{t("contextVault.allEntries")}</span><small>{claims.length}</small></button>
        <div className="context-vault-rail-label">{t("contextVault.categoriesLabel")}</div>
        <div className="context-vault-category-group">{categories.map((item) => <button key={item} className={`context-vault-category${category === item && !tag ? " active" : ""}`} onClick={() => { setCategory(item); setTag(null); }}><span>{t(`contextVault.categories.${item}`)}</span><small>{claims.filter((claim) => claim.category === item).length}</small></button>)}</div>
        {tags.length > 0 && <><div className="context-vault-rail-label">{t("contextVault.tags")}</div><div className="context-vault-tag-rail">{tags.map((item) => <button key={item} className={`context-vault-category${tag === item ? " active" : ""}`} onClick={() => { setCategory("all"); setTag(item); }}><span>#{item}</span><small>{claims.filter((claim) => claim.tags.includes(item)).length}</small></button>)}</div></>}
