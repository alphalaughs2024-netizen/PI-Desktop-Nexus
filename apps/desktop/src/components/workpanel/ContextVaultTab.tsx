...
type ContextVaultViewState = "no-project" | "loading" | "ready" | "empty" | "search-empty" | "editing" | "busy" | "error";
...
export function ContextVaultTab() {
  ...
  const actionsDisabled = claimsLoading || busy !== null;
  const isBusy = busy !== null;
  const canCreate = Boolean(projectPath) && !claimsLoading && !isBusy;
  const canImport = Boolean(projectPath) && !claimsLoading && !isBusy;
  const canExport = Boolean(projectPath) && !claimsLoading && !isBusy && claims.length > 0;
  const operationLabel = claimsLoading ? t("common.loading") : busy === "import" ? "Importing…" : busy === "importApply" ? "Applying import…" : busy === "export" ? "Exporting…" : busy === "save" ? "Saving claim…" : busy === "review" ? "Applying review…" : busy === "recheck" ? "Rechecking claim…" : busy === "delete" ? "Deleting claim…" : null;
  const hasFilter = Boolean(query || category !== "all" || tag);
  const viewState: ContextVaultViewState = !projectPath ? "no-project" : claimsLoading ? "loading" : isEditing ? "editing" : isBusy ? "busy" : error && claims.length === 0 ? "error" : hasFilter && visibleClaims.length === 0 ? "search-empty" : claims.length === 0 ? "empty" : "ready";
  ...
  return <section className="context-vault-tab">
    <div className="context-vault-workspace">
      ...
      <div className="context-vault-main">
        <header className="context-vault-toolbar"><label className="context-vault-search-shell"><IconSearch size={15} aria-hidden="true" /><input aria-label={t("contextVault.search")} className="context-vault-search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t("contextVault.search")} /></label><div className="context-vault-actions"><button type="button" className="context-vault-action context-vault-action-primary" disabled={!canCreate} aria-describedby={!canCreate ? "context-vault-operation-status" : undefined} onClick={startClaim}><IconPlus size={15} aria-hidden="true" />{t("contextVault.newClaim")}</button><button type="button" className="context-vault-action context-vault-action-secondary" disabled={!canImport} aria-describedby={!canImport ? "context-vault-operation-status" : undefined} onClick={() => void previewImport()}>{t("contextVault.import")}</button><button type="button" className="context-vault-action context-vault-action-secondary" disabled={!canExport} aria-describedby={!canExport ? "context-vault-operation-status" : undefined} onClick={() => void exportVault()}>{t("contextVault.export")}</button></div></header>
        ...
        {viewState === "loading" ? <div className="context-vault-empty"><h2>{t("common.loading")}</h2></div> : viewState === "error" ? <div className="context-vault-empty context-vault-error-state"><h2>Unable to load project knowledge</h2><button type="button" className="context-vault-action context-vault-action-secondary" onClick={() => void refresh()}>Retry</button></div> : viewState === "search-empty" ? <div className="context-vault-empty context-vault-search-empty"><h2>{t("contextVault.noMatchingClaims")}</h2><p>Try another search or clear the active filter.</p><button type="button" className="context-vault-action context-vault-action-secondary" onClick={clearFilters}>Clear filters</button></div> : viewState === "empty" ? <div className="context-vault-empty"><div className="context-vault-empty-orbit" aria-hidden="true"><span /><span /><span /><div className="context-vault-empty-icon"><IconBookOpen size={32} /></div></div><h2>{t("contextVault.emptyTitle")}</h2><p>{t("contextVault.emptyBody")}</p><button type="button" className="context-vault-action context-vault-action-primary context-vault-empty-action" disabled={!canCreate} aria-describedby={!canCreate ? "context-vault-operation-status" : undefined} onClick={startClaim}><IconPlus size={15} aria-hidden="true" />{t("contextVault.addFirst")}</button><div className="context-vault-empty-tools"><button type="button" className="context-vault-action context-vault-action-secondary" disabled={!canCreate} aria-describedby={!canCreate ? "context-vault-operation-status" : undefined} onClick={startUserDecision}>{t("contextVault.userDecision")}</button><button type="button" className="context-vault-action context-vault-action-secondary" disabled={!canImport} aria-describedby={!canImport ? "context-vault-operation-status" : undefined} onClick={() => void previewImport()}>{t("contextVault.import")}</button></div></div> : <div className={`context-vault-grid${isEditing ? " is-editing" : ""}`}>
