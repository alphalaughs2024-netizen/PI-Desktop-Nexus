...
import { derivePromptInspectorState, filterPromptLifecycle, formatPromptDuration, groupPromptLifecycle, normalizePromptLifecycle, promptEventLabel, promptReasonLabel, promptRoutineLabel, promptSectionLabel, resolvePromptProviderModel, safeContextVaultSummary, PROMPT_INSPECTOR_FILTER_LABELS } from "../../lib/prompt-inspector";
...
function collapseRoutineEvents(events: readonly PromptLifecycleEvent[]) {
  ...
  for (const event of events) {
    ...
    if (routine && previous && previous.events.every((candidate) => candidate.kind === event.kind) && event.ts - previous.events.at(-1)!.ts <= 2_000) {
