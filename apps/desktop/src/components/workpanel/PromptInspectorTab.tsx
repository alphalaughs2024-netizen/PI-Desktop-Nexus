...
export function PromptInspectorTab() {
  ...
  return <div className="prompt-inspector-tab">
    ...
    <div className="prompt-inspector-summary"><span>{composition.sections.length} sections</span><span>{composition.durationMs.toFixed(1)}ms</span><span>{composition.reloadReason}</span></div>
    ...
    <div className="prompt-inspector-timeline">{timeline.map((event) => <article key={event.id}><strong>{event.kind.replaceAll("_", " ")}</strong><span>{new Date(event.ts).toLocaleTimeString()}</span>{event.reason ? <small>{event.reason}</small> : null}</article>)}</div>
  </div>;
}
