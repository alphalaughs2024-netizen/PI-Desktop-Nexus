...
export function UsagePage() {
  ...
  const costRows = useMemo(() => { const pricedTotal = costEstimate.rows.reduce((sum, row) => sum + (row.cost ?? 0), 0); const catalogModels = Object.values(providerModels).flat(); return costEstimate.rows.map((row) => { const modelInfo = catalogModels.find((model) => model.modelId === row.id || model.displayName === row.label); const provider = configuredProviders.find((item) => item.id === (modelInfo as any)?.providerId || item.vendorKey === (modelInfo as any)?.providerId); const statusLabel = row.cost == null ? "Unpriced" : row.cost === 0 ? "Free" : "Catalog estimate"; return { ...row, providerLabel: provider?.name ?? provider?.vendorKey ?? "Provider unavailable", baseUrl: provider?.baseUrl, modelLabel: row.label, pricedSpendPercent: row.cost != null && pricedTotal > 0 ? (row.cost / pricedTotal) * 100 : undefined, statusLabel }; }); }, [costEstimate.rows, providerModels, configuredProviders]);
  const costGroups = useMemo(() => { const groups = new Map<string, typeof costRows>(); for (const row of costRows) { const key = `${row.providerLabel}|${row.baseUrl ?? ""}`; groups.set(key, [...(groups.get(key) ?? []), row]); } return [...groups.entries()].map(([key, rows]) => ({ key, label: key.split("|")[0], baseUrl: key.split("|")[1], rows: rows.sort((a, b) => (b.cost ?? -1) - (a.cost ?? -1) || a.modelLabel.localeCompare(b.modelLabel)) })); }, [costRows]);
