...
export function UsagePage() {
  ...
  return (
    <div className="settings-stack usage-page">
      ...
      <section className="usage-dashboard-section usage-provider-accounts"><div className="usage-section-heading"><div><span className="usage-section-kicker">Future provider wiring</span><h2>Provider accounts</h2><p>Account spend and quota are separate from Nexus session estimates.</p></div></div><div className="usage-provider-grid"><div className="usage-provider-card"><div><strong>XKIRO</strong><span>Provider account</span></div><em>Not connected</em><div className="usage-provider-slots"><span>Wallet <b>—</b></span><span>Spend window <b>—</b></span><span>Reset <b>—</b></span></div><button type="button" disabled>Connect provider account</button></div><div className="usage-provider-card"><div><strong>OpenRouter</strong><span>Provider account</span></div><em>Coming next</em><div className="usage-provider-slots"><span>Credits <b>—</b></span><span>Requests <b>—</b></span><span>Updated <b>—</b></span></div><button type="button" disabled>Connect provider account</button></div><div className="usage-provider-card usage-provider-card-muted"><div><strong>OpenAI · Anthropic · Groq</strong><span>Organization and quota surfaces</span></div><em>Unavailable</em><p>Provider connections will appear here when billing support is enabled.</p></div></div></section>
