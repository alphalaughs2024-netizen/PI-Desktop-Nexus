...
export function ProjectGroupCreateDialog({ onClose, onSave }: { onClose: () => void; onSave: (name: string) => void }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <form className="modal-dialog" role="dialog" aria-modal="true" aria-labelledby="project-group-create-title" onSubmit={(event) => { event.preventDefault(); const value = name.trim(); if (value) { onSave(value); onClose(); } }}>
