...
export function ProjectCollectionPicker({ projectPath, collections, selected, onClose, onToggle, onCreate, projects = [], assignmentCollectionId, onAssign, anchor }: Props) {
  ...
  useEffect(() => {
    ...
    const onPointerDown = (event: PointerEvent) => { if (panelRef.current && !panelRef.current.contains(event.target as Node)) onClose(); };
  ...
  const placement = anchor ? { top: Math.max(12, Math.min(anchor.top, window.innerHeight - 24)), left: Math.max(12, Math.min(anchor.left, window.innerWidth - 352)) } : undefined;
  return <section ref={panelRef} className="collection-picker-popover" role="dialog" aria-modal="true" aria-labelledby="project-collection-picker-title" style={placement}>
