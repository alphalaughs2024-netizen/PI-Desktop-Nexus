...
test("renderer surfaces the PI Desktop Nexus fork brand", () => {
  assert.match(english, /shellName:\s*"PI Desktop Nexus"/);
  assert.match(chinese, /shellName:\s*"PI Desktop Nexus"/);
  assert.match(english, /placeholder:\s*"Ask PI Desktop Nexus to help with anything"/);
  assert.match(chinese, /placeholder:\s*"让 PI Desktop Nexus 帮你做任何事"/);
