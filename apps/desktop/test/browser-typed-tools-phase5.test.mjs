...
const plugin = await readFile(new URL("../resources/plugins/pi.browser/main.js", import.meta.url), "utf8");
...
test("Phase 5 keeps the legacy Browser union as compatibility", () => {
  assert.match(plugin, /name: "Browser"/);
  assert.match(plugin, /planSafeActions/);
});
