...
#### E2E-024IAR：工作流条与会话结算回归（ADR 0228）

- **先决条件**：一个已激活工作流、已排队后续提示的 Nexus 会话；提供方夹具能够完成、失败或中止回合；视觉检查时启用 Twilight Mountains。
- **步骤**：1) 在一轮运行期间排队两条提示。2) 结束该回合并等待持久化结算。3) 确认仅第一条排队提示启动，然后对第二条重复。4) 打开工作流“检查”，并将窗口调整到窄宽度。5) 切换应用语言后重新打开卡片。
- **预期**：队列在终端持久化结算前保持阻塞，随后严格按 FIFO 顺序且仅一次恢复。工作流卡片保持在 shell 上方，其三个操作具有可见的轮廓和焦点状态，“检查”显示为全宽、可读的表面，不出现黑色矩形或布局碰撞。标签和回退文案跟随所选语言；忽略和设置保留现有会话/导航行为。
- **链接规格**：`03-runtime/01-ipc-protocol.md` §12b.1、ADR 0228
- **接受**：E（工具和权限）+ G（代理指导）
- **状态**：单元/源代码契约覆盖；Electron 视觉旅程草稿
...
- **步骤**：1) 打开会话。2) 确认已完成回合答案下方只有模型徽章、没有
  ...
  7) 空闲时点击该回合的重试。8) 确认没有用量的会话仍提供重试但不显示
  输入框检查器。
- **预期**：有模型 id 时答案下方显示模型徽章。有用量后，紧凑上下文
  ...
  发送最近的用户提示，并在回合运行时禁用；body 级面板保持在视口内，
  不被输入框或记录滚动裁剪，并跟随滚动或调整大小；复制仍不包含思考
  文本。
- **链接规格**：`04-ux/08-component-spec.md`、
...
- **步骤**：1) 将两个文件拖到 Composer 上方，确认 Composer 外壳显示文件拖放轮廓。2) 在已知草稿以及权限、上下文用量和模型/推理控件可见时，从文件资源管理器将图片夹具放入 Composer；检查叶名芯片并发送提示。3) 拖放纯文本或 URL，确认应用不会将其变成附件。4) 在 home Composer 重复，并检查新建会话的暂存根。
- **预期**：文件拖放遵循与剪贴板文件相同的有界 `composer/pasteFiles` 路线：会话物化、暂存副本、MIME/图片分类、结构化附件元数据，以及干净的工作区。捕获阶段守卫会阻止混合拖放源无关的 `text/plain` 元数据或 Chromium 随后的 `insertFromDrop` 事件与附件芯片一起出现在可编辑区域。已知草稿其余内容保持不变：权限、上下文用量以及模型/推理控件文字永远不会出现在其中。离开或放下后轮廓清除。非文件拖放保持浏览器原生行为，Composer 外没有文件拖放目标。
...
- **步骤**：在“外观”选择 Twilight Mountains，依次选择低、中和高模糊，并确认每次选择后当前聊天背景立即变化；重启应用后再切换到基础主题。
...
#### E2E-NEXUS-005：原生窗口控件和 Context Vault 边界

- **步骤**：在 Windows/Linux 打开对话并展开工作面板，检查右上角最小化、
  最大化/还原和关闭按钮；在没有会话时打开面板菜单；随后创建会话并在项目
  切换时保留 Context Vault 面板。
- **预期**：原生控件始终固定在窗口右上角且可点击，标题栏拖拽区域不会覆盖
  控件。没有会话时 Context Vault 显示说明并保持禁用；创建会话后按钮可用，
  保留面板始终解析活动会话的项目，不显示错误项目或黑色空白表面。
- **状态**：由桌面源码契约测试覆盖；桌面行程仍为草稿。
