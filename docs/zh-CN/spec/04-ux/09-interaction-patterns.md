...
- 激活 `TooltipButton` 后会在动作派发后关闭可见工具提示；指针重新进入时可再次显示，
  且不会改变原有动作处理器。
- 披露 V 形在扩展时旋转。减少运动禁用
...
## 8a。输入框自动完成、粘贴和文件拖放（D123–D125、D197、D209、D262、D362、ADR 0131）
...
- 包含一个或多个 OS `File` 对象的粘贴或拖放会在 Composer 中被拦截。字符数不超过持久化的 `largePasteThreshold`（默认 600）时，
  纯文本粘贴保持原生；超过阈值时会被拦截并转换为会话临时文件引用。
- 文件拖放会在捕获阶段和导入处理器中阻止。这会阻止混合拖放载荷中无关的
  `text/plain` 元数据，以及 Chromium 随后的 `beforeinput` `insertFromDrop` 插入进入可编辑区域，
  同时保留文件附件流程；放下处理器仍通过有界会话暂存桥导入文件。现有草稿文本保持不变，
  Composer 控件永远不会成为草稿内容。文件拖到 Composer 上方时，现有输入框外壳显示强调色轮廓；
  离开或放下时该提示会清除。文字和 URL 拖放保持浏览器原生行为，不会变为附件；Composer 外部没有文件拖放目标。
- 传输字节时，文本区域是只读的并公开
  `aria-busy="true"`；发送和自动完成控件被禁用。
- Electron main 在原始会话的暂存下保存来自粘贴或拖放的有界字节
