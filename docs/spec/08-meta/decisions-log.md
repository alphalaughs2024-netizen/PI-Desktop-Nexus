...
| D395 | Browser becomes a built-in core capability | **Supersedes D333 and ADR 0170 for ownership: core registers exactly one Browser tool/view/Preview capability; `pi.browser.*` remains a non-duplicating compatibility API; Browser availability does not depend on plugin activation, pluginViews, or ToolSearch. Preserve Main-owned sandboxed WebContentsView, CDP allowlist, workspace containment, Plan/Agent policy, session-bound request context, snapshot generations, bounded payloads, and no-replay ambiguous mutation semantics.** | Browser is core to Nexus agent workflows; plugin packaging adds availability, discovery, and lifecycle friction without improving the already host-owned security boundary. |

| D406 | Browser Phase 9 legacy removal | **Remove `pi.browser` from bundled product assets and host-core manifest reconciliation after clean-profile, upgrade-profile, security, E2E, and third-party compatibility gates pass. Retain `pi.browser.*` only as a Main-owned compatibility adapter; legacy tab/reference normalization remains. No Browser visual HTML, plugin tool registration, or plugin lifecycle ownership remains in shipped packages.** | The Browser capability is already core-authoritative; keeping the old package creates duplicate ownership and stale lifecycle paths. |
| D407 | Work Panel docked/maximized presentation contract | **The Work Panel remains a fixed-width in-flow dock by default and gains a transient renderer-owned `docked`/`maximized` presentation. Maximized is an in-window bounded Nexus frame; it never resizes the OS window or creates another BrowserWindow/WebContentsView. The same resource/session/Browser identity survives dock, maximize, close, and reopen according to existing lifecycle rules. Shell owns presentation, geometry, transitions, focus, and frame surfaces; resources own content chrome and measured content bounds. Background sessions cannot retarget the visible shell.** | Browser needs more space for inspection without becoming a separate application or permanently displacing conversation. The contract is shared by Browser, Files, Terminal, Sources, Diff, and Artifacts. |

| D408 | Browser Phase 1 direct discoverability | **Browser is a native Nexus Work Panel launcher and empty-state action independent of `pluginViews`, `pi.browser`, BrowserPreview, or ToolSearch. Activating it opens/reuses the active session's canonical `browser` / `core://browser` tab; legacy tab normalization and singleton guest identity remain unchanged. Rich Browser chrome and maximized presentation are deferred to later phases.** | A core capability that cannot be manually opened is not discoverable; the launcher must be available before richer Browser UI is implemented. |

| D409 | Browser Phase 2 presentation implementation | **The renderer now implements a shared Work Panel frame with transient docked/maximized state. Docked remains the fixed in-flow column; maximized is a bounded in-window frame using the same resource subtree. Shell controls own Maximize/Dock/Close; Browser guest identity, session state, and security boundaries remain unchanged.** | Browser needs focused workspace room without becoming a separate application or creating a second native guest. |

| D410 | Browser Phase 3 component decomposition | **BrowserCoreTab is a structured renderer resource: toolbar, readiness strip, guest surface, stable operation status, safe error notice, empty state, and diagnostics mount point. Only BrowserGuestSurface measures/publishes guest bounds. No backend/API/security behavior changes.** | Separating resource responsibilities lets later Browser chrome evolve without destabilizing the Main-owned guest contract or accessibility tree. |

| D411 | Browser Phase 4 functional toolbar | **BrowserToolbar now exposes typed Back/Forward/Reload/Stop, safe address entry, bounded screenshot, safe external-open, and overflow actions. Renderer owns draft/focus/pending presentation; Main/Broker retain URL, capability, session, external-open, and payload policy.** | Browser needs essential user affordances without becoming a Chrome clone or weakening the existing Main-owned security boundary. |

| D412 | Browser Phase 5 safe view state and recovery | **BrowserViewState separates lifecycle/source presentation from navigation BrowserState and carries only sanitized readiness, source, safe location/title, recovery, stable error, and suggested-action fields. Renderer state surfaces use explicit retry/reopen/diagnostics behavior; recovery never replays Browser operations.** | A navigation object cannot safely describe lifecycle failure, policy, source, or recovery. Explicit safe state makes Browser availability actionable without exposing page or host internals. |

| D413 | Browser Phase 6 session-safe contextual diagnostics | **Browser view state and diagnostics are retained and published per session. The Browser panel ignores other-session events. Diagnostics are projected to safe bounded fields and rendered in a non-modal docked/maximized overlay with deterministic copy and focus restoration.** | Background Browser activity must not retarget the visible session, and useful diagnostics must not expose Browser/page/host internals or become a dashboard. |

| D414 | Browser Phase 7 smooth guest surface | **BrowserGuestSurface is the sole normalized, generation-guarded bounds publisher. Dock/maximize, diagnostics, readiness, narrow layout, focus, and reduced-motion polish remain renderer-only; the Main-owned guest is never animated, recreated, or reloaded by presentation changes.** | Physical smoothness and visual polish must not weaken Browser identity, security, session ownership, or guest lifecycle boundaries. |

| D415 | Browser Phase 8 accessibility and interaction polish | **Browser controls, overflow, diagnostics, state surfaces, and WorkPanel shell controls use stable semantic labels/roles, deterministic keyboard/focus behavior, reduced-motion-safe transitions, narrow-layout hit targets, and semantic contrast tokens. The single status/alert contract remains singular.** | Browser must be fully usable by keyboard and assistive technology without changing its Main-owned security or guest architecture. |
